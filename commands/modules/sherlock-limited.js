 ...
 * - Changed colon parsing
 * - Modified to make configuring colon for 24 hour time optional
 */
...
var Sherlock = (function() {
    ...
    var patterns = {
            ...
            // Now removed:
            relativeDateStr: "(?!)",
            // relativeDateStr: "((?:next|last|this) (?:week|month|year)|tom(?:orrow)?|tmrw|tod(?:ay)?|tonight|day after (?:tom(?:orrow)?|tmrw)|yest(?:erday)?|day before yest(?:erday)?)",
            ...
            inRelativeTime: /(?!)/g,
            // inRelativeTime: /\b(\d{1,2} ?)(h(?:our|r)?|m(?:in(?:ute)?)?)s? ?(ago|old)?\b/,
            ...
            // Requires colon:
            internationalTimeColon: /\b(?:(0[0-9]|1[3-9]|2[0-3]):([0-5]\d))\b/,

            //12hr times, 12:00 AM - 11:59 AM, 12:00 PM - 11:59 PM
            // 5, 12pm, 5:00, 5:00pm, at 5pm, @3a
            explicitTime: /(?:@ ?)?\b(?:at |from )?(1[0-2]|[0-2]?[1-9])(?::([0-5]\d))? ?([ap]\.?m?\.?)?(?:(o['\s]{0,2})clock)?\b/,
        ...
        parser = function(str, time, startTime, require24HourColon = true) {
            ...
            // parse time
            if (timeMatch = matchTime(strNummed, time, startTime, require24HourColon))
                str = str.replace(new RegExp(helpers.numToStr(timeMatch)), "$TIME$");

            ret.eventTitle = str;

            // if time data not given, then this is an all day event
            ret.isAllDay = Boolean(dateMatch && !timeMatch && !dateMatch.match(/^(?:right )?now$|^tonight$/));

            // check if date was parsed
            ret.isValidDate = Boolean(dateMatch || timeMatch);
        ...
        matchTime = function(str, time, startTime, require24HourColon = true) {
            ...
            if (match = str.match(new RegExp(patterns.explicitTime.source, "g"))) {
                ...
                if (match.length <= 2 && str.trim().length > 2)
                    ...
                    if (!meridian && match[1].startsWith("0") && str.match(require24HourColon ? patterns.internationalTimeColon : patterns.internationalTime)) {
                    ...
                    matchedHour = hour;
                    matchedMin = min;
                    matchedHasMeridian = Boolean(meridian);
            ...
            if (matchConfidence < 4) {
                ...
                else if (match = str.match(require24HourColon ? patterns.internationalTimeColon : patterns.internationalTime)) {
    ...
    // plus anything Watson adds on...
        parse: function(str, now, require24HourColon = true) {
            ...
            while (!ret.startDate) {
                ...
                if ((result = parser(tokens[0], date, null, require24HourColon)) !== null) {
                    if (result.isAllDay)
                    // set to midnight
                        date.setHours(0, 0, 0);
            ...
            while (!ret.endDate) {
                if (tokens.length > 1) {
                    ...
                    // parse the end date
                    if ((result = parser(tokens[2], date, ret, require24HourColon)) !== null) {
                        if (ret.isAllDay)
                        // set to midnight
                            date.setHours(0, 0, 0);
        ...
        _can_pass_now_in_parse: true // Help avoid confusion with modified sherlock and normal sherlock
    };
})();
