...
module.exports = {
    ...
    data: {
        // Slash command data
        command: new SlashCommandBuilder().setName("ban")
            .setDescription("Ban a user, temporarily if desired")
            .addUserOption(option =>
                option.setName("target").setDescription("Who to ban?")
                    .setRequired(true)
            )
            ...
            .addIntegerOption(option =>
                option.setName("days").setDescription("How many days until unbanned?")
                    .setMinValue(1)
                    .setMaxValue(365)
            )
            .addIntegerOption(option =>
                option.setName("hours").setDescription("How many hours until unbanned?")
                    .setMinValue(1)
                    .setMaxValue(23)
            )
    ...
    async execute(cmd, context) {
        ...
        if (temp) {
            comps = [new ActionRowBuilder().addComponents(new ButtonBuilder().setStyle(ButtonStyle.Danger)
                .setLabel("Unban Now")
                .setCustomId(`unban-${targetInServer.id}`))];
        }
