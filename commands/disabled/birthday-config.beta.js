...
module.exports = {
    data: {
        command: new SlashCommandBuilder().setName("birthday-config")
            .setDescription("Configure how birthdays are handled in this server")
            .addSubcommand(command =>
                command.setName("channel_config").setDescription("Setup a channel for notifications on people's birthdays")
                    .addChannelOption(option =>
                        option.setName("channel").setDescription("The channel to post birthday notifications into")
                            .addChannelTypes(ChannelType.GuildText)
                            .setRequired(true)
                    )
                    .addBooleanOption(option =>
                        option.setName("active").setDescription("Should I be running the birthday notifications?")
                            .setRequired(true)
                    )
            ...
            .addSubcommand(command =>
                command.setName("role-config").setDescription("Set roles for users under or over a specific age")
                    .addRoleOption(option =>
                        option.setName("role").setDescription("The role to configure")
                            .setRequired(true)
                    )
                    .addIntegerOption(option =>
                        option.setName("must_be_over").setDescription("Only apply this role to users who are over X age (13 for no limit)")
                            .setMinValue(13)
                            .setMaxValue(120)//We refuse to process users who try to set their year for 13 or younger
                    )
                    .addIntegerOption(option =>
                        option.setName("must_be_under").setDescription("Only apply this role to users who are under X age (120 for no limit)")
                            .setMinValue(14)
                            .setMaxValue(120)//There's no point in processing an under age at 13, nobody will ever hit it, so we make the min 14.
                    )
                    .addBooleanOption(option =>
