...
module.exports = {
    data: {
        // Slash command data
        command: new SlashCommandBuilder().setName("auto_roles")
            .setDescription("Setup a message with auto roles")
            .addStringOption(option =>
                option.setName("message").setDescription("The message to be sent with the role options")
                    .setRequired(true)
            )
    ...
    async onbutton(cmd, context) {
        ...
        if (cmd.customId == "role-addOption") {
            ...
            for (let role of cmd.values) {
                ...
                tempRow.push(new ButtonBuilder().setCustomId("autoRole-" + role)
                    .setLabel(roleInQuestion.name)
