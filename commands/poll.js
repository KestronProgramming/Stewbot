...
module.exports = {
    data: {
        // Slash command data
        command: new SlashCommandBuilder().setName("poll")
            .setDescription("Make a poll with automatically tracked options")
            .addStringOption(option =>
                option.setName("prompt").setDescription("The prompt (We'll set options in a minute)")
                    .setRequired(true)
            )
            .addBooleanOption(option =>
                option.setName("legend").setDescription("Should this chart include a legend?")
                    .setRequired(false)
            )
            .addBooleanOption(option =>
                option.setName("labels").setDescription("Should this chart include labels?")
                    .setRequired(false)
            )
            .addStringOption(option =>
                option.setName("chart").setDescription("What type of chart should I use?")
                    .setRequired(false)
                    .addChoices(
    ...
    async execute(cmd, context) {
        ...
        const configurationMessage = await cmd.followUp({
            ...
            components: [new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId("poll-addOption")
                    .setLabel("Add a poll option")
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder().setCustomId("poll-delOption")
                    .setLabel("Remove a poll option")
                    .setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId("poll-publish")
                    .setLabel("Publish the poll")
